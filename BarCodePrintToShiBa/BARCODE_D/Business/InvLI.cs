...
{
    public class InvLI : BizObject, IAuditing
    {
        #region Constructor
        /// <summary>
        /// Constructor
        ...
        public SunlikeDataSet GetData(string usr, string LzID, string LzNo, bool OnlyFillSchema, bool HasDsc)
        {
            ...
            SunlikeDataSet _ds = _dbinvLI.GetData(LzID, LzNo, OnlyFillSchema);
            ...
            if (!OnlyFillSchema)
            {
                if (usr != null && !String.IsNullOrEmpty(usr))
                {
                    ...
                    DataTable _dtMf = _ds.Tables["MF_LZ1"];
            ...
            DataTable _dtTflz = _ds.Tables["TF_LZ1"];
            if (HasDsc)
            {
                //取得特征分段数据
            ...
            if (_ds.Tables["MF_LZ1"].Rows.Count > 0)
            {

                foreach (DataRow dr in _ds.Tables["TF_LZ1"].Rows)
                {
                    if ((_ds.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString() == "1") ||
                        (dr["BIL_ID"].ToString() == "EP"))
                        Load_Body_Data1(dr);
                    else
                        Load_Body_Data2(dr);
                }
        ...
        private void Load_Body_Data2(DataRow dr)
        {
            ...
            //foreach (DataRow dr in ds.Tables["TF_LZ1"].Rows)
            //{
            string _bilId = dr["BIL_ID"].ToString();
            string _ckNo = dr["PS_NO"].ToString();
            string _tblStr = "BN,BB";
            #region 借出借入单
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                DRPBN _drpbn = new DRPBN();
                _sourceDs = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilId, _ckNo, false);
                HTable = _sourceDs.Tables["MF_BLN"];
                BTable = _sourceDs.Tables["TF_BLN"];
            }
            #endregion
            #region 销退折单
            _tblStr = "SA,SB,SD,PC,PB,PD";
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                DRPPC _drpsa = new DRPPC();

                _sourceDs = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilId, _ckNo, false, false);
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                ...
                _sourceDs = _drpio.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilId, _ckNo, false);
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                ...
                _sourceDs = _mtnfin.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilId, _ckNo);
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TB_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,CHK_MAN,VOH_NO,TAX_ID  from MF_TB"
                                + " where TB_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TB Where TB_NO='" + _ckNo + "'";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TB";
                _sourceDs.Tables[1].TableName = "TF_TB";

                HTable = _sourceDs.Tables["MF_TB"];
                BTable = _sourceDs.Tables["TF_TB"];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TC_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,CHK_MAN,VOH_NO,TAX_ID  from MF_TC"
                                + " where TC_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TC Where TC_NO='" + _ckNo + "'";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TC";
                _sourceDs.Tables[1].TableName = "TF_TC";

                HTable = _sourceDs.Tables["MF_TC"];
                BTable = _sourceDs.Tables["TF_TC"];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TD_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,CHK_MAN,VOH_NO,TAX_ID  from MF_TD"
                                + " where TD_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TD Where TD_NO='" + _ckNo + "'";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TD";
                _sourceDs.Tables[1].TableName = "TF_TD";

                HTable = _sourceDs.Tables["MF_TD"];
                BTable = _sourceDs.Tables["TF_TD"];
            ...
            if (_sourceDs.Tables.Contains("TF_BLN"))
            {
                foreach (DataRow bldr in BTable.Rows)
                {
                    ...
                    if (string.IsNullOrEmpty(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["CUR_ID"].ToString()))
                        _rAmt = _rAmt + ConvertDecimal(bldr, "AMTN_RNT_NET") + ConvertDecimal(bldr, "TAX_RNT");
                    else
                        if ((dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TAX_ID"].ToString() == "3") &&
                                (!string.IsNullOrEmpty(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["CUR_ID"].ToString()))
                            )
            ...
            if (_sourceDs.Tables.Contains("TF_PSS") || _sourceDs.Tables.Contains("TF_MFIN1") ||
                _sourceDs.Tables.Contains("TF_TB") || _sourceDs.Tables.Contains("TF_TC") || _sourceDs.Tables.Contains("TF_TD"))
            {
            ...
            int PreNo = 1;
            string backStr = "SB,SD,IB,DT,TC";
            if (
                 (dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VERSION"].ToString() == "1") &&
        ...
        private void Load_Body_Data1(DataRow dr)
        {
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
              ...
              //  DRPBN _drpbn = new DRPBN();
                ...
                //_sourceDs = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilId, _ckNo, false);
                DataRow[] bodyAry = _sourceDs.Tables["TF_BLN"].Select("EST_ITM=" + dr["EST_ITM"].ToString());
                if (bodyAry.Length > 0)
                {
                    HTable = _sourceDs.Tables["MF_BLN"];
                    bRow = bodyAry[0];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                ...
                _sourceDs.Tables[1].TableName = "TF_MFIN1"; DataRow[] bodyAry = _sourceDs.Tables["TF_MFIN1"].Select("PRE_ITM=" + dr["EST_ITM"].ToString());
                if (bodyAry.Length > 0)
                {
                    HTable = _sourceDs.Tables["MF_MFIN"];
                    bRow = bodyAry[0];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TB_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TB"
                                + " where TB_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TB Where TB_NO='" + _ckNo + "' and EST_ITM='" + dr["EST_ITM"].ToString() + "'";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TB";
                _sourceDs.Tables[1].TableName = "TF_TB";

                DataRow[] bodyAry = _sourceDs.Tables["TF_TB"].Select("EST_ITM=" + dr["EST_ITM"].ToString());
                if (bodyAry.Length > 0)
                {
                    HTable = _sourceDs.Tables["MF_TB"];
                    bRow = bodyAry[0];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TC_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TC"
                                + " where TC_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TC Where TC_NO='" + _ckNo + "' and EST_ITM='" + dr["EST_ITM"].ToString() + "'";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TC";
                _sourceDs.Tables[1].TableName = "TF_TC";

                DataRow[] bodyAry = _sourceDs.Tables["TF_TC"].Select("EST_ITM=" + dr["EST_ITM"].ToString());
                if (bodyAry.Length > 0)
                {
                    HTable = _sourceDs.Tables["MF_TC"];
                    bRow = bodyAry[0];
            ...
            if (_tblStr.IndexOf(_bilId) > -1)
            {
                string _sql = "Select TD_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TD"
                                + " where TD_NO='" + _ckNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TD Where TD_NO='" + _ckNo + "' and EST_ITM='" + dr["EST_ITM"].ToString() + "";
                Query _query = new Query();
                _sourceDs = _query.DoSQLString(_sql);
                _sourceDs.Tables[0].TableName = "MF_TD";
                _sourceDs.Tables[1].TableName = "TF_TD";

                DataRow[] bodyAry = _sourceDs.Tables["TF_TD"].Select("EST_ITM=" + dr["EST_ITM"].ToString());
                if (bodyAry.Length > 0)
                {
                    HTable = _sourceDs.Tables["MF_TD"];
                    bRow = bodyAry[0];
            ...
            int PreNo = 1;
            string backStr = "SB,SD,IB,PD,PB,DT,TC";
            if (
                 (dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VERSION"].ToString() == "1") &&
                 ((backStr.IndexOf(_bilId) > -1) || (_bilId == "IM" && dr["IMTAG"].ToString() == "O"))
                )
            ...
            if (_bilId != "EP")
                dr["QTY_OLD"] = ConvertDecimal(dr, "QTY");
            if (string.IsNullOrEmpty(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["CHK_MAN"].ToString()))
            {
        ...
        {
            DataTable _dtMf = ds.Tables["MF_LZ1"];
            DataTable _dtTf = ds.Tables["TF_LZ1"];
            if (_dtMf.Rows.Count > 0)
            {
        ...
        //    DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
        //    _dbinvLI.UpIsZero = Chk_Up_Zero;
        //    //SunlikeDataSet _ds = _dbinvLI.GetDataForBil(parameters);
        ...
        //    SunlikeDataSet _ds = _dbinvLI.GetDataForBil(_bilid,
        //                                                _chkDate,
        //                                                _fDate,
        //                                                _eDate,
        //                                                _fPsNo,
        ...
        //        else
        //            if (_taxID == "3" && !string.IsNullOrEmpty(_cur_id))  //应税外加且有币别
        //            {
        //                decimal _myAmt = 0;
        ...
        {
            int preNo = 1;
            string BackStr = "SB,SD,IB,KB,PB,PD,TC,DT";
            if ((ver == "1") &&
        ...
        private DataTable CreateTmpTable()
        {
            ...
            _dt.Columns.Add(new DataColumn("PS_NO")); //来源单号
        ...
        {
            // TODO:  Add DrpTaxAa.Approve implementation
            try
            {
                SunlikeDataSet _ds = this.GetData("", bil_id, bil_no);
                if (_ds.Tables["MF_LZ1"].Rows.Count > 0)
                {
                    DataRow _drHead = _ds.Tables["MF_LZ1"].Rows[0];
                    DataTable _dtBody = _ds.Tables["TF_LZ1"];

                    foreach (DataRow dr in _dtBody.Rows)  //更新来源单的资讯 扣掉原先表身金额
                    {
                        if (dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString() == "1" || dr["BIL_ID"].ToString() == "EP")
                            this.UpdateBILAMTN_NET_FP(dr, true);
                        else
                            this.UpdateBILAMTN_NET_FPHead(dr, true);
                        AppendToTF_LZ_EP(dr, bil_id, bil_no, dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString());
                    }
                    DbInvLI _dbinvli = new DbInvLI(Comp.Conn_DB);
                    ...
                    if (bil_id == "LP")
                    {
                        ...
                        _dbinvli.UpdateChkMan(bil_id, bil_no, chk_man, cls_dd);
                        ...
                        {
                            Bills _bills = new Bills();
                            _bills.DelRcvPay("2", _drHead["RP_NO"].ToString());
                        }
                        #endregion
                    }
                    else
        ...
        private string RollBack(string bil_id, string bil_no, bool isUpdateHead)
        {
            ...
            try
            {
                ...
                {
                    DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                    _dbinvLI.UpdateChkMan(bil_id, bil_no, "", DateTime.MinValue);
                    _dbinvLI.UpdateArpNo(bil_id, bil_no, "", 0, 0, 0);
                    _dbinvLI.DeleteToTF_LZ_EP(bil_id, bil_no);
                }
        ...
        private string UpdateMon(DataRow dr)
        {
            ...
            {
                DataRow[] _drSel = dr.Table.DataSet.Tables["TF_LZ1"].Select();
        ...
        {
            if (ds.Tables.Count > 0 && ds.Tables.Contains("MF_LZ1") && ds.Tables["MF_LZ1"].Rows.Count > 0 && ds.Tables["MF_LZ1"].Rows[0].RowState == System.Data.DataRowState.Deleted)
            {
                this.UpdateMfArp(ds.Tables["MF_LZ1"].Rows[0]);
            }
        ...
        {
            string _psID, _usr;
            DataRow _dr = ChangedDS.Tables["MF_LZ1"].Rows[0];
            ...
            _ht["MF_LZ1"] = "LZ_ID,LZ_NO,LZ_DD,PAY_DD,CUS_NO,INV_NO,CUR_ID,Exc_RTO," +
                           ...
                           "FJ_NUM,SYS_DATE,TURN_ID,VOH_CHK,CAS_NO,TASK_ID," +
                           "RP_NO,PRT_USR,BIL_TYPE,QC_SW";

            _ht["TF_LZ1"] = "LZ_ID,LZ_NO,ITM,PS_NO,AMT,AMTN_NET,TAX,PRD_NO,EST_ITM,PAY_DD," +
                            "BIL_ID,TAX_RTO,PRD_MARK,QTY,UP,COMPOSE_IDNO,BAT_NO,CUS_OS_NO,SUP_PRD_NO," +
                            "UNIT,PRD_NAME,CUS_NO";
            //判断是否走审核流程
            ...
            if (!ChangedDS.HasErrors)
            {
                ...
                if (!String.IsNullOrEmpty(_UpdUsr))
                {
                    ...
                    DataTable _dtMf = ChangedDS.Tables["MF_LZ1"];
                    if (_dtMf.Rows.Count > 0)
                    {
                        string _bill_Dep = _dtMf.Rows[0]["DEP"].ToString();
                        string _bill_Usr = _dtMf.Rows[0]["USR"].ToString();
        ...
        protected override void BeforeUpdate(string tableName, StatementType statementType, DataRow dr, ref UpdateStatus status)
        {
            ...
            #endregion
            else if (tableName == "TF_LZ1" && dr.RowState != DataRowState.Deleted)
            {

                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString();
                Prdt _prdt = new Prdt();
                if (dr["BIL_ID"].ToString() != "EP" && dr["BIL_ID"].ToString() != "PD" && dr["BIL_ID"].ToString() != "DT")
                {
                    //检查货品代号
                    if (!_prdt.IsExist(_usr, dr["PRD_NO"].ToString(), Convert.ToDateTime(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["LZ_DD"])) && dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString() == "1")
                    {
                ...
                if (!string.IsNullOrEmpty(dr["VOH_ID"].ToString()) &&
                    !string.IsNullOrEmpty(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VOH_ID"].ToString())
                    )
                {
                    throw new SunlikeException("RCID=INV.INVIK.ACCVOHID,PARAM=" + dr["PS_NO"].ToString());
                }
        ...
        protected override void BeforeDsSave(DataSet ds)
        {
            ...
            { return; }
            //#region 单据追踪
            //DataTable _dt = ds.Tables["MF_LZ1"];
            ...
            if (ds.Tables["MF_LZ1"].Rows.Count > 0 && ds.Tables["MF_LZ1"].Rows[0].RowState == DataRowState.Deleted)
            {
                Bills _bills = new Bills();  //by zb 2007-05-14
                if (!string.IsNullOrEmpty(ds.Tables["MF_LZ1"].Rows[0]["RP_NO", DataRowVersion.Original].ToString()))
                    _bills.DelRcvPay("2", ds.Tables["MF_LZ1"].Rows[0]["RP_NO", DataRowVersion.Original].ToString());
                this.UpdateMfArp(ds);
            }
        ...
        {
            dr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0];
            DataTable _dtBody = dr.Table.DataSet.Tables["TF_LZ1"];
            ...
            if (dr.RowState != DataRowState.Deleted && _amt != 0)
            {
                ...
                //_arpNo = _arp.UpdateMfArp("2", "2", dr["LZ_ID"].ToString(), dr["LZ_NO"].ToString(), Convert.ToDateTime(dr["LZ_DD"]), dr["BIL_TYPE"].ToString(),
                ...
                if (_arpNo != dr["ARP_NO"].ToString())
                {
                    ...
                    {
                        DbInvLI _dbInvli = new DbInvLI(Comp.Conn_DB);
                        _dbInvli.UpdateArpNo(dr["LZ_ID"].ToString(), dr["LZ_NO"].ToString(), _arpNo, _amtn, _tax, _amt);
                    }
                    dr["ARP_NO"] = _arpNo;
                }
                dr["AMTN_NET"] = _amtn;
            ...
            if (statementType == StatementType.Update)
            {
                ...
                if (this._reBuildVohNo)
                {
                    ...
                    if (!string.IsNullOrEmpty(dr["VOH_ID"].ToString()))
                    {
                        ...
                        if (string.Compare("LP", _psId) == 0 || string.Compare("LI", _psId) == 0)
                        {
                            _getVoh = _compInfo.VoucherInfo.GenVoh.PC_INVOICE;
                        }
                        if (_getVoh)
                        {
        ...
        public void UpdateVohNo(string lzId, string lzNo, string vohNo)
        {
            ...
            DbInvLI _lz = new DbInvLI(Comp.Conn_DB);
            _lz.UpdateVohNo(lzId, lzNo, vohNo);
        }
        #endregion
        /// <summary>
        /// 保存单据之后的动作
            ...
            if (!_isRunAuditing)
            {
                if (tableName == "MF_LZ1")
                {
                    ...
                    if (tableName == "TF_LZ1")
                    {
                        ...
                        if (dr.RowState == DataRowState.Deleted)
                        {
                            _turnId = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID", DataRowVersion.Original].ToString();
                        ...
                        {
                            _turnId = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString();
                            _bilID = dr["BIL_ID"].ToString();
                            _bilNo = dr["PS_NO"].ToString();

                        }

                        if (_turnId == "1" || _bilID == "EP")
                        {
                            if (statementType == StatementType.Insert)
                            {
                                this.UpdateBILAMTN_NET_FP(dr, true);
        ...
        protected override void AfterDsSave(DataSet ds)
        {
            ...
            {
                DataRow dr = ds.Tables["MF_LZ1"].Rows[0];
                //因前面有切凭证，故这里不需要重新切制
            ...
            try
            {
                if (turnId == "2")
                {
                    ...
                    else //MF_PSS
                    {
                        ...
                        string _sql = "Select CUR_ID,OS_NO,Dis_Cnt,AMTN_EP,AMTN_EP1,CLSCK,PS_ID,PS_NO,EP_NO,EP_NO1,TAX_ID,CLSLZ,VOH_ID,VOH_NO  from MF_PSS Where PS_ID='" + osId + "' and PS_NO='" + osNo + "';" +
                    ...
                    if ((LocateOK_H) && (LocateOK_B))
                    {
                        ...
                        if (osId == "CK")
                        {
                            ...
                            {
                                _str = "Select CLSCK,AMTN_EP,AMTN_EP1,EP_NO,EP_NO1,TAX_ID,CLSLZ frrm MF_PSS Where PS_ID='" + osId + "' and PS_NO='" + osNo + "'";
                ...
                else  //turn_id==1
                {
                    ...
                    if ((LocateOK_H) && (LocateOK_B))
                    {
                        ...
                        if (osId == "CK")
                        {
                            ...
                            {
                                string _tmpSql = "Select CUR_ID,OS_NO,Dis_Cnt,AMTN_EP,EP_NO1,PS_ID,PS_NO,EP_NO,EP_NO1,CLSLZ,VOH_ID,VOH_NO  from MF_PSS Where PS_ID='" + bodydr["OS_ID"].ToString() + "' and PS_NO='" + bodydr["OS_NO"].ToString() + "';" +
        ...
        private void UpdateBILAMTN_NET_FPHead(DataRow dr, bool IsAdd)
        {
            ...
            if (IsAdd)
            {
                _sver = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VERSION"].ToString();
                _osId = dr["BIL_ID"].ToString();

                _osNo = dr["PS_NO"].ToString();

                _lz_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["LZ_NO"].ToString();
                _inv_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["INV_NO"].ToString();
                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString();
                _voh_id = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VOH_ID"].ToString();
            }
            else
            {
                _sver = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VERSION", DataRowVersion.Original].ToString();
                _osId = dr["BIL_ID", DataRowVersion.Original].ToString();
                _osNo = dr["PS_NO", DataRowVersion.Original].ToString();

                _preNo = -1;
                _lz_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["LZ_NO", DataRowVersion.Original].ToString();
                _inv_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["INV_NO", DataRowVersion.Original].ToString();
                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString();
                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VOH_ID", DataRowVersion.Original].ToString();
            }
            #region 栏位定义
            string fAMT_FP = "";
            string fAMTN_NET_FP = "";
            ...
            if (mfPSSStr.IndexOf(_osId) > -1)
            {
                DRPPC _drpsa = new DRPPC();
                if (IsAdd)
                    _ds = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _osId, _osNo, false, false);
                else
                    _ds = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _osId, _osNo, false, false);
                mfdr = _ds.Tables["MF_PSS"].Rows[0];
                bodydt = _ds.Tables["TF_PSS"];
            }
            #region 借出,借出還入單据
            string mfBilLnStr = "BN,BB";
            if (mfBilLnStr.IndexOf(_osId) > -1)
            {
                DRPBN _drpbn = new DRPBN();
                if (IsAdd)
                    _ds = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _osId, _osNo, false);
                else
                    _ds = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _osId, _osNo, false);
            ...
            if (mfBilLnStr.IndexOf(_osId) > -1)
            {
                ...
                if (IsAdd)
                    _ds = _drpio.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _osId, _osNo, false);
                else
                    _ds = _drpio.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _osId, _osNo, false);
            ...
            if (mfBilLnStr.IndexOf(_osId) > -1)
            {
                string _sql = "Select TB_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TB"
                            + " where TB_NO='" + _osNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TB Where TB_NO='" + _osNo + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TB";
                _ds.Tables[1].TableName = "TF_TB";
                if (_ds.Tables["MF_TB"].Rows.Count == 0)
                    return;
                mfdr = _ds.Tables["MF_TB"].Rows[0];
                bodydt = _ds.Tables["TF_TB"];
            ...
            if (mfBilLnStr.IndexOf(_osId) > -1)
            {
                string _sql = "Select TC_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TC"
                            + " where TC_NO='" + _osNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TC Where TC_NO='" + _osNo + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TC";
                _ds.Tables[1].TableName = "TF_TC";
                if (_ds.Tables["MF_TC"].Rows.Count == 0)
                    return;
                mfdr = _ds.Tables["MF_TC"].Rows[0];
                bodydt = _ds.Tables["TF_TC"];
            ...
            if (mfBilLnStr.IndexOf(_osId) > -1)
            {
                string _sql = "Select TD_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,VOH_ID,EXC_RTO,VOH_NO,CHK_MAN,TAX_ID  from MF_TD"
                            + " where TD_NO='" + _osNo + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TD Where TD_NO='" + _osNo + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TD";
                _ds.Tables[1].TableName = "TF_TD";
                if (_ds.Tables["MF_TD"].Rows.Count == 0)
                    return;
                mfdr = _ds.Tables["MF_TD"].Rows[0];
                bodydt = _ds.Tables["TF_TD"];

            }
            #endregion

            if (!string.IsNullOrEmpty(_voh_id) && mfdr.Table.Columns.Contains("VOH_ID") && !string.IsNullOrEmpty(mfdr["VOH_ID"].ToString()))
            {
                throw new SunlikeException("RCID=INV.INVIK.ACCVOHID,PARAM=" + dr["PS_NO"].ToString());
            }
            //--------------加其他類型的來源單
            ...
            if (IsAdd)
            {
                ...
                {
                    DbInvLI _dbInvLI = new DbInvLI(Comp.Conn_DB);
                    _dbInvLI.UpdateInv_noForMFPss(_lz_no, _osNo);
                }
            ...
            else
            {
                if (mfdr.Table.DataSet.Tables.Contains("MF_PSS"))
                {
                    ...
                    {
                        DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                        _dbinvLI.UpdateMFPssforACC_FP_NO(_lz_no, _osNo);
                    }
                mfdr[fLZ_CLS_ID] = "F";
            ...
            if (mfPSSStr.IndexOf(_osId) > -1)
            {
                ...
                if (IsAdd)
                    _dbinvLI.UpdateMFARPINVNO(mfdr["PS_ID"].ToString(), mfdr["PS_NO"].ToString(), "", mfdr["INV_NO"].ToString());
                else
                    _dbinvLI.UpdateMFARPINVNO(mfdr["PS_ID"].ToString(), mfdr["PS_NO"].ToString(), "D", mfdr["INV_NO"].ToString());

                DbDRPPC _dbPc = new DbDRPPC(Comp.Conn_DB);
                _dbPc.UpdateInvIkHeadData(mfdr["TURN_ID"].ToString(), mfdr["LZ_CLS_ID"].ToString(), mfdr["CLSLZ"].ToString(),
            ...
            if (tblstr.IndexOf(_osId) > -1)
            {
                ...
                if (IsAdd)
                    _dbinvLI.UpdateMFARPINVNO(mfdr["BL_ID"].ToString(), mfdr["BL_NO"].ToString(), "", mfdr["INV_NO"].ToString());
                else
                    _dbinvLI.UpdateMFARPINVNO(mfdr["BL_ID"].ToString(), mfdr["BL_NO"].ToString(), "D", mfdr["INV_NO"].ToString());
            ...
            {
                DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                _dbinvLI.UpDateMFTBHead2(mfdr, _osNo); //滙总合并
            ...
            {
                DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                _dbinvLI.UpDateMFTCHead2(mfdr, _osNo); //滙总合并
            ...
            {
                DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                _dbinvLI.UpDateMFDTHead2(mfdr, _osNo); //滙总合并
        ...
        private void UpdateBILAMTN_NET_FP(DataRow dr, bool IsAdd)
        {
            ...
            if (IsAdd)
            {
                ...
                _turn_ID = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID"].ToString();
                _inv_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["INV_NO"].ToString();

                _lzNo = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["LZ_NO"].ToString();
                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString();
            }
            else
            {
                ...
                _turn_ID = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["TURN_ID", DataRowVersion.Original].ToString();
                _inv_no = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["INV_NO", DataRowVersion.Original].ToString();
                _sver = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VERSION", DataRowVersion.Original].ToString();

                _lzNo = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["LZ_NO", DataRowVersion.Original].ToString();
                _usr = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString();
                _voh_id = dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["VOH_ID", DataRowVersion.Original].ToString();
            }
            #region 栏位定义
            string fAMT_FP = "";
            string fAMTN_NET_FP = "";
            ...
            if (mfPSSStr.IndexOf(_bilID) > -1)
            {
                ...
                if (IsAdd)
                {
                    _ds = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilID, _bilNO, false, false);
                    bodyAry = _ds.Tables["TF_PSS"].Select("PRE_ITM=" + dr["EST_ITM"].ToString());

                }
                else
                {
                    _ds = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _bilID, _bilNO, false, false);
            ...
            if (mfBilLnStr.IndexOf(_bilID) > -1)
            {
                ...
                if (IsAdd)
                {
                    _ds = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilID, _bilNO, false);
                    bodyAry = _ds.Tables["TF_BLN"].Select("PRE_ITM=" + dr["EST_ITM"].ToString());

                }
                else
                {
                    _ds = _drpbn.GetData("", dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _bilID, _bilNO, false);
            ...
            if (mfBilLnStr.IndexOf(_bilID) > -1)
            {
                ...
                if (IsAdd)
                {
                    _ds = _drpio.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilID, _bilNO, false);
                    bodyAry = _ds.Tables["TF_IC"].Select("KEY_ITM=" + dr["EST_ITM"].ToString());

                }
                else
                {
                    _ds = _drpio.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR", DataRowVersion.Original].ToString(), _bilID, _bilNO, false);
            ...
            {
                DataRow[] bodyAry;
                string _sql = "Select TB_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,EXC_RTO,VOH_ID,CHK_MAN,VOH_NO,TAX_ID  from MF_TB"
                                + " where TB_NO='" + _bilNO + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,EST_ITM,AMTN_NET,TAX,QTY,AMT From TF_TB Where TB_NO='" + _bilNO + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TB";
                _ds.Tables[1].TableName = "TF_TB";
                ...
                if (IsAdd)
                {
                    bodyAry = _ds.Tables["TF_TB"].Select("EST_ITM=" + dr["EST_ITM"].ToString());
                }
                else
                {
                    bodyAry = _ds.Tables["TF_TB"].Select("EST_ITM=" + dr["EST_ITM", DataRowVersion.Original].ToString());
                }
                bodyDt = _ds.Tables["TF_TB"];
                if (bodyAry.Length > 0)
                    bodydr = bodyAry[0];
            ...
            {
                DataRow[] bodyAry;
                string _sql = "Select TC_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,EXC_RTO,VOH_ID,CHK_MAN,VOH_NO,TAX_ID  from MF_TC"
                                + " where TC_NO='" + _bilNO + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,PRE_ITM,AMTN_NET,TAX,QTY,AMT From TF_TC Where TC_NO='" + _bilNO + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TC";
                _ds.Tables[1].TableName = "TF_TC";
                ...
                if (IsAdd)
                {
                    bodyAry = _ds.Tables["TF_TC"].Select("PRE_ITM=" + dr["EST_ITM"].ToString());
                }
                else
                {
                    bodyAry = _ds.Tables["TF_TC"].Select("PRE_ITM=" + dr["EST_ITM", DataRowVersion.Original].ToString());
                }
                bodyDt = _ds.Tables["TF_TC"];
                if (bodyAry.Length > 0)
                    bodydr = bodyAry[0];
            ...
            {
                DataRow[] bodyAry;
                string _sql = "Select TD_NO,INV_NO,CAS_NO,TASK_ID,LZ_CLS_ID,TURN_ID,AMTN_NET_CLS,AMT_CLS,TAX_CLS,QTY_CLS,CUR_ID,EXC_RTO,VOH_ID,CHK_MAN,VOH_NO,TAX_ID  from MF_TD"
                                + " where TD_NO='" + _bilNO + "';";
                _sql += " Select AMTN_NET_FP,AMT_FP,TAX_FP,QTY_FP,PRE_ITM,AMTN_NET,TAX,QTY,AMT From TF_TD Where TD_NO='" + _bilNO + "'";
                Query _query = new Query();
                _ds = _query.DoSQLString(_sql);
                _ds.Tables[0].TableName = "MF_TD";
                _ds.Tables[1].TableName = "TF_TD";
                ...
                if (IsAdd)
                {
                    bodyAry = _ds.Tables["TF_TD"].Select("PRE_ITM=" + dr["EST_ITM"].ToString());
                }
                else
                {
                    bodyAry = _ds.Tables["TF_TD"].Select("PRE_ITM=" + dr["EST_ITM", DataRowVersion.Original].ToString());
                }
                bodyDt = _ds.Tables["TF_TD"];
                if (bodyAry.Length > 0)
                    bodydr = bodyAry[0];
            ...
            if (IsAdd)
            {
                if (!string.IsNullOrEmpty(_voh_id) && !string.IsNullOrEmpty(mfdr["VOH_ID"].ToString()))
                {
                    throw new SunlikeException("RCID=INV.INVIK.ACCVOHID,PARAM=" + dr["PS_NO"].ToString());
                }
            }
            ...
            if (_bilstr.IndexOf(_bilID) < 0)
            {
                if (IsAdd)
                {
                    if (mfdr.Table.DataSet.Tables.Contains("MF_PSS"))
                    {
                        ...
                        if (bodydr.Table.DataSet.Tables.Contains("TF_CK"))
                        {
                            if (bodydr["OS_ID"].ToString() == "PC")
                            {
                                DRPPC _drpsa = new DRPPC();
                                SunlikeDataSet _saDataSet = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilID, _bilNO);
                                if (_saDataSet.Tables["MF_PSS"].Rows.Count > 0)
                                {
                                    DbDRPPC _dbPc = new DbDRPPC(Comp.Conn_DB);
                                    _dbPc.UpdateMFPSSACC_FP_NO(_lzNo, _saDataSet.Tables["MF_PSS"].Rows[0]["PS_ID"].ToString(), _saDataSet.Tables["MF_PSS"].Rows[0]["PS_NO"].ToString());
                                }
                            }
                        }
                ...
                else  //isAdd =false
                {
                    ...
                    if (bodydr.Table.DataSet.Tables.Contains("TF_CK") && bodydr["OS_ID"].ToString() == "PC")
                    {
                        DRPPC _drpsa = new DRPPC();
                        SunlikeDataSet _saDataSet = _drpsa.GetData(dr.Table.DataSet.Tables["MF_LZ1"].Rows[0]["USR"].ToString(), _bilID, _bilNO);
                        if (_saDataSet.Tables["MF_PSS"].Rows.Count > 0)
                        {
                            DbDRPPC _dbPc = new DbDRPPC(Comp.Conn_DB);
                            _dbPc.UpdateMFPSSACC_FP_NO("", _saDataSet.Tables["MF_PSS"].Rows[0]["PS_ID"].ToString(), _saDataSet.Tables["MF_PSS"].Rows[0]["PS_NO"].ToString());
                        }
                    ...
                    string _nouseInvNo = "IO,IB,TJ";
                    if (_nouseInvNo.IndexOf(_bilID) == -1 && clearInv_NO)
                        mfdr["INV_NO"] = "";
                    if (mfdr.Table.DataSet.Tables.Contains("MF_PSS"))
                    {
                        string _tblStr = "SA,PC";
                        if (_tblStr.IndexOf(_bilID) > -1 && (mfdr["ACC_FP_NO"].ToString() == _lzNo && clearInv_NO))
                            mfdr["ACC_FP_NO"] = "";
                ...
                if (tblstr.IndexOf(_bilID) > -1)
                {
                    ...
                    _dbPc.UpdateInvIkHeadData(mfdr["TURN_ID"].ToString(), mfdr["LZ_CLS_ID"].ToString(), mfdr["CLSLZ"].ToString(),
                        mfdr["ACC_FP_NO"].ToString(), mfdr["INV_NO"].ToString(), ConvertDecimal(mfdr, "AMT_CLS"), ConvertDecimal(mfdr, "AMTN_NET_CLS"),
                        ConvertDecimal(mfdr, "TAX_CLS"), ConvertDecimal(mfdr, "QTY_CLS"), mfdr["PS_ID"].ToString(), mfdr["PS_NO"].ToString());
                    _dbPc.UpdateInvIkBodyData(ConvertDecimal(bodydr, "AMT_FP"), ConvertDecimal(bodydr, "AMTN_NET_FP"), ConvertDecimal(bodydr, "TAX_FP"),
                ...
                {
                    DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                    _dbinvLI.UpDateMFTBHead(mfdr, _bilNO);
                    _dbinvLI.UpDateTFTBBody(bodydr, _bilNO);
                }
                #endregion
                #region 托外退回单 TC
                tblstr = "TC";
                ...
                {
                    DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                    _dbinvLI.UpDateMFTCHead(mfdr, _bilNO);
                    _dbinvLI.UpDateTFTCBody(bodydr, _bilNO);
                }
                #endregion
                #region 托外退回单 TC
                tblstr = "DT";
                ...
                {
                    DbInvLI _dbinvLI = new DbInvLI(Comp.Conn_DB);
                    _dbinvLI.UpDateMFDTHead(mfdr, _bilNO);
                    _dbinvLI.UpDateTFDTBody(bodydr, _bilNO);
                }
                #endregion
            ...
            else
            {
                ...
                if (IsAdd)
                {
                    _dbinvLi.UpdateMFARPINVNOForEXP(bodydr["EP_ID"].ToString(), bodydr["EP_NO"].ToString(), Convert.ToInt32(bodydr["ITM"]), "", bodydr["INV_NO"].ToString());
                    bodydr["INV_NO"] = _inv_no;
                }
                else
                {
                    _dbinvLi.UpdateMFARPINVNOForEXP(bodydr["EP_ID"].ToString(), bodydr["EP_NO"].ToString(), Convert.ToInt32(bodydr["ITM"]), "D", bodydr["INV_NO"].ToString());
                    bodydr["INV_NO"] = "";
                }
        ...
        private bool needCls2(DataRow mfdr, DataTable bydt, DataRow tflzdr, string fAMTN_NET_CLS, string fTAX_CLS, string fAMT_CLS)
        {
            ...
            string _osNo = tflzdr["PS_NO"].ToString();
        ...
        /// <param name="_bilID">TF_LZ1.BIL_ID</param>
        ...
        {
            DbInvLI _invDbLi = new DbInvLI(Comp.Conn_DB);
            SunlikeDataSet _ds = _invDbLi.GetData(lzId, lzNo, false);
            DataTable _mflzTable = _ds.Tables["MF_LZ1"];
            Arp _arp = new Arp();
            try
            {
        ...
        /// 判断来源单是否存在TF_LZ1
        /// </summary>
        /// <param name="lzId">TF_LZ1的单据别</param>
        ...
        public SunlikeDataSet GetInTfLz(string lzId, string bilId, string ckno)
        {
            ...
            DbInvLI _dbinvli = new DbInvLI(Comp.Conn_DB);
            return _dbinvli.GetInTfLz(lzId, bilId, ckno);
        ...
        public SunlikeDataSet GetInTfLz(string bilId, string ckno)
        {
            ...
            DbInvLI _dbinvli = new DbInvLI(Comp.Conn_DB);
            return _dbinvli.GetInTfLz(bilId, ckno);
        ...
        {
            DbInvLI _dbInvLi = new DbInvLI(Comp.Conn_DB);
            return _dbInvLi.GetAmtn_EPForQuery(lzId, lzNo, epId, lzItm);
        }
        ...
        public SunlikeDataSet GetAllInvNoForBil(Dictionary<string, object> ht)
        {
            ...
            return _dbinvli.GetAllInvNoForBil(bilId, bilNo);
        }
    }
}
