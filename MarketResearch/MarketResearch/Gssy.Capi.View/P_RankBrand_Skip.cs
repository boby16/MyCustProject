...
{
	public class P_RankBrand_Skip : Page, IComponentConnector
	{
		[Serializable]
		[CompilerGenerated]
		...
		private string NoneCode = _003F487_003F._003F488_003F(";ĸ");
		...
		private void _003F80_003F(object _003F347_003F, RoutedEventArgs _003F348_003F)
		{
			...
			((P_RankBrand_Skip)/*Error near IL_0a58: Stack underflow*/).Text_Width = (int)/*Error near IL_0a58: Stack underflow*/;
			...
			if (SurveyHelper.AutoFill)
			{
				...
				listButton = autoFill.MultiButton(oQuestion.QDefine, listButton, listButton, btnOrders.Count);
		...
		private void _003F99_003F(object _003F347_003F, EventArgs _003F348_003F)
		{
			...
			//IL_00f5: Incompatible stack heights: 1 vs 0
			//IL_0106: Incompatible stack heights: 0 vs 2
			//IL_0137: Incompatible stack heights: 0 vs 2
			if (PageLoaded)
			{
				WrapPanel wrapPanel = wrapPanel2;
				int button_Type = Button_Type;
				if ((int)/*Error near IL_00b9: Stack underflow*/ != 0)
				{
					if (Button_Type == 2)
					{
					}
					((WrapPanel)/*Error near IL_004c: Stack underflow*/).Orientation = Orientation.Vertical;
				}
				else
				{
					Visibility computedVerticalScrollBarVisibility = scrollmain.ComputedVerticalScrollBarVisibility;
					if ((int)/*Error near IL_00ce: Stack underflow*/ == 2)
					{
						wrapPanel.Orientation = Orientation.Vertical;
						((P_RankBrand_Skip)/*Error near IL_001c: Stack underflow*/).Button_Type = 2;
					}
					else
					{
				...
				if (oQuestion.QDefine.CONTROL_TOOLTIP.Trim() != _003F487_003F._003F488_003F(""))
				{
					...
					string cONTROL_TOOLTIP = ((QMatrixSingle)/*Error near IL_007a: Stack underflow*/).QDefine.CONTROL_TOOLTIP;
					int num = ((P_RankBrand_Skip)/*Error near IL_0084: Stack underflow*/)._003F96_003F(cONTROL_TOOLTIP);
					if (num == 1)
					{
					...
					else if (num > 1)
					{
						...
						GridLength width = new GridLength((double)/*Error near IL_013c: Stack underflow*/);
						((ColumnDefinition)/*Error near IL_0141: Stack underflow*/).Width = width;
					}
				}
		...
		{
			//IL_036c: Incompatible stack heights: 0 vs 1
			//IL_037c: Incompatible stack heights: 0 vs 1
			//IL_037d: Incompatible stack heights: 0 vs 1
			//IL_04a7: Incompatible stack heights: 0 vs 1
			//IL_04b7: Incompatible stack heights: 0 vs 1
			//IL_04b9: Incompatible stack heights: 0 vs 1
			Style style = (Style)FindResource(_003F487_003F._003F488_003F("Xůɥ\u034aѳը\u0656ݰ\u087a८\u0a64"));
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					((WrapPanel)/*Error near IL_0382: Stack underflow*/).Orientation = (Orientation)/*Error near IL_0382: Stack underflow*/;
					...
					((TextBlock)/*Error near IL_04be: Stack underflow*/).Foreground = (Brush)/*Error near IL_04be: Stack underflow*/;
		...
		private bool _003F87_003F()
		{
			...
			Brush foreground = (Brush)FindResource(_003F487_003F._003F488_003F("\\Źɯ\u037aѻբ٢\u0747\u0876ॶ\u0a71୩"));
			Brush foreground2 = (Brush)new BrushConverter().ConvertFromString(_003F487_003F._003F488_003F("RŬɪͶѤ"));
			oQuestion.SelectedCodeCount = 0;
			bool flag = false;
			int num = 0;
			for (int num2 = oQuestion.SelectedCode.Count - 1; num2 >= 0; num2--)
			{
				num = num2;
				if (oQuestion.SelectedCode[num2] != _003F487_003F._003F488_003F(""))
				{
					oQuestion.SelectedCodeCount++;
					flag = true;
				}
				else if (flag)
				{
					...
					foreach (TextBlock txtOrder in txtOrders)
					{
						if (oQuestion.SelectedCode[num3] == _003F487_003F._003F488_003F(""))
						{
							txtOrder.Foreground = foreground2;
						}
						else
			...
			if (!flag)
			{
				if (MessageBox.Show(SurveyMsg.MsgRB_info1 + Environment.NewLine + Environment.NewLine + SurveyMsg.MsgPassCheck, SurveyMsg.MsgCaption, MessageBoxButton.YesNo, MessageBoxImage.Asterisk, MessageBoxResult.No).Equals(MessageBoxResult.No))
				{
					return true;
				}
				return false;
			}
			if (oQuestion.SelectedCodeCount < oQuestion.SelectedCode.Count)
			{
				if (MessageBox.Show(SurveyMsg.MsgRB_info2 + Environment.NewLine + Environment.NewLine + SurveyMsg.MsgPassCheck, SurveyMsg.MsgCaption, MessageBoxButton.YesNo, MessageBoxImage.Asterisk, MessageBoxResult.No).Equals(MessageBoxResult.No))
				{
					return true;
				}
				return false;
		...
		private List<VEAnswer> _003F88_003F()
		{
			...
			str += MyNav.QName_Add;
			if (oQuestion.SelectedCodeCount == 0)
			{
				string text = oQuestion.QuestionName + _003F487_003F._003F488_003F("\\ŐȰ");
				VEAnswer vEAnswer = new VEAnswer();
				vEAnswer.QUESTION_NAME = text;
				vEAnswer.CODE = NoneCode;
				list.Add(vEAnswer);
				SurveyHelper.Answer = text + _003F487_003F._003F488_003F("<") + NoneCode;
				text = oQuestion.QuestionName + _003F487_003F._003F488_003F("]ł") + NoneCode;
				vEAnswer = new VEAnswer();
				vEAnswer.QUESTION_NAME = text;
				vEAnswer.CODE = _003F487_003F._003F488_003F("0");
				list.Add(vEAnswer);
				text = str + _003F487_003F._003F488_003F("\\ŐȰ");
				vEAnswer = new VEAnswer();
				vEAnswer.QUESTION_NAME = text;
				vEAnswer.CODE = oQuestion.QCircleDetails[0].CODE;
				list.Add(vEAnswer);
			}
			else
			{
				...
				foreach (string item in oQuestion.SelectedCode)
				{
					...
					string text2 = oQuestion.QuestionName + _003F487_003F._003F488_003F("]œ") + oQuestion.QCircleDetails[num].CODE;
					VEAnswer vEAnswer2 = new VEAnswer();
					vEAnswer2.QUESTION_NAME = text2;
					vEAnswer2.CODE = item;
					list.Add(vEAnswer2);
					SurveyHelper.Answer = SurveyHelper.Answer + _003F487_003F._003F488_003F("-") + text2 + _003F487_003F._003F488_003F("<") + item;
					text2 = oQuestion.QuestionName + _003F487_003F._003F488_003F("]ł") + item;
					vEAnswer2 = new VEAnswer();
					vEAnswer2.QUESTION_NAME = text2;
					vEAnswer2.CODE = oQuestion.QCircleDetails[num].CODE;
					list.Add(vEAnswer2);
					text2 = str + _003F487_003F._003F488_003F("]œ") + (num + 1).ToString();
					vEAnswer2 = new VEAnswer();
					vEAnswer2.QUESTION_NAME = text2;
					vEAnswer2.CODE = oQuestion.QCircleDetails[num].CODE;
					list.Add(vEAnswer2);
		...
		public void InitializeComponent()
		{
			...
			IL_000b:
			_contentLoaded = true;
			Uri resourceLocator = new Uri(_003F487_003F._003F488_003F("\0ũɞ\u035fђԄ٪\u0749ࡗ\u094fਞ\u0b47\u0c4c\u0d4f๑ཏ\u1071ᅻታ፨ᐴᕬᙰ\u177dᡠ\u1939\u1a65ᭋᱡᵳṿύ\u206dⅼ≬⍢⑯╕♺❣⡮⥶⨫⭼Ɫ\u2d6f\u2e6d"), UriKind.Relative);
		...
		void IComponentConnector.Connect(int _003F349_003F, object _003F350_003F)
		{
			...
			{
			case 1:
				((P_RankBrand_Skip)_003F350_003F).Loaded += _003F80_003F;
				((P_RankBrand_Skip)_003F350_003F).LayoutUpdated += _003F99_003F;
				break;
			case 2:
