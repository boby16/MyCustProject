...
{
    public class DbInvLI:DbObject
    {
        #region Construction
        public DbInvLI(string connstr)
            : base(connstr)
        { }
        #endregion
        ...
        public SunlikeDataSet GetData(string LzID, string LzNo, bool OnlyFillSchema)
        {
            ...
            string _sql = "Select A.LZ_ID,A.LZ_NO,A.LZ_DD,A.PAY_DD,A.CUS_NO,A.INV_NO,A.CUR_ID,A.Exc_RTO," +
                        ...
                        "A.FJ_NUM,A.SYS_DATE,A.TURN_ID,A.VOH_CHK,A.CAS_NO,A.TASK_ID," +
                        "A.RP_NO,A.PRT_USR,A.BIL_TYPE,A.QC_SW ,B.NAME As CUS_NAME,D.NAME AS DEPNAME," +
                        "C.NAME As SAL_NAME," +
                        ...
                        " left join MF_ARP F on F.ARP_ID = '2' AND F.OPN_ID = '2' AND F.ARP_NO=A.ARP_NO" +
                        ...
                        " Select A.LZ_ID,A.LZ_NO,A.ITM,A.PS_NO,A.AMT,A.AMTN_NET,A.TAX,A.PRD_NO, "+
                        " A.EST_ITM,A.PAY_DD,BIL_ID,A.TAX_RTO,A.PRD_MARK,A.QTY,A.UP,"+
                        " A.COMPOSE_IDNO,A.BAT_NO,A.CUS_OS_NO,A.SUP_PRD_NO,A.UNIT, "+
                        " A.PRD_NAME,A.CUS_NO,"+
            ...
            if (OnlyFillSchema)
                this.FillDatasetSchema(_sql,_ds,new string[]{"MF_LZ1","TF_LZ1"},_spc);
            else
                this.FillDataset(_sql,_ds,new string[]{"MF_LZ1","TF_LZ1"},_spc);
            _ds.Tables["TF_LZ1"].Columns["UNIT_NAME"].ReadOnly = true;
            //设定PK，因为用了left join以后，PK会取不到
            DataColumn[] _dca = new DataColumn[2];
            _dca[0] = _ds.Tables["MF_LZ1"].Columns["LZ_ID"];
            _dca[1] = _ds.Tables["MF_LZ1"].Columns["LZ_NO"];
            _ds.Tables["MF_LZ1"].PrimaryKey = _dca;

            _dca = new DataColumn[3];
            _dca[0] = _ds.Tables["TF_LZ1"].Columns["LZ_ID"];
            _dca[1] = _ds.Tables["TF_LZ1"].Columns["LZ_NO"];
            _dca[2] = _ds.Tables["TF_LZ1"].Columns["ITM"];
            _ds.Tables["TF_LZ1"].PrimaryKey = _dca;
            //表头和表身关联
            DataColumn[] _dca1 = new DataColumn[2];
            _dca1[0] = _ds.Tables["MF_LZ1"].Columns["LZ_ID"];
            _dca1[1] = _ds.Tables["MF_LZ1"].Columns["LZ_NO"];
            DataColumn[] _dca2 = new DataColumn[2];
            _dca2[0] = _ds.Tables["TF_LZ1"].Columns["LZ_ID"];
            _dca2[1] = _ds.Tables["TF_LZ1"].Columns["LZ_NO"];
        ...
        string rightWhere, string CHKAUD)
        {
            ...
            { 
                ...
                case "EP":
                    ...
                    break;
                case "TB":
                    SQLStr = GetSqlStrForMF_TB(BIL_ID, CHK_DATE, FDATE, EDATE,
                         FPS_NO, EPS_NO, CUS_NO, INCUS, TAX_ID, TURN_ID,
                         Zhang_ID, DEP, SAL_NO, CUR_ID, CAS_NO, TASK_ID,
                         CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE);
                    break;
                case "TC":
                    SQLStr = GetSqlStrForMF_TC(BIL_ID, CHK_DATE, FDATE, EDATE,
                         FPS_NO, EPS_NO, CUS_NO, INCUS, TAX_ID, TURN_ID,
                         Zhang_ID, DEP, SAL_NO, CUR_ID, CAS_NO, TASK_ID,
                         CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE);
                    break;
                case "DT":
                    SQLStr = GetSqlStrForMF_DT(BIL_ID, CHK_DATE, FDATE, EDATE,
                         FPS_NO, EPS_NO, CUS_NO, INCUS, TAX_ID, TURN_ID,
                         Zhang_ID, DEP, SAL_NO, CUR_ID, CAS_NO, TASK_ID,
                         CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE);
                    break;
        ...
        private string GetSqlStrForMF_DT(string BIL_ID, string CHK_DATE, string FDATE, string EDATE, string FPS_NO, string EPS_NO, string CUS_NO, string INCUS, string TAX_ID, string TURN_ID, string Zhang_ID, string DEP, string SAL_NO, string CUR_ID, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE)
        {
            StringBuilder _sb = new StringBuilder();
            _sb.Append(" SELECT B.PRD_NO,G.Name As PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
            _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TD_DD AS PAY_DD,'DT' As BIL_ID,A.TD_NO As BIL_NO, ");
            _sb.Append(" 0 As Dis_Cnt,'' As OS_NO,A.ZHANG_ID,0 As AMTN_EP,0 As AMTN_ER,UP=0,B.BAT_NO,G.CHK_TAX,'' As CUS_OS_NO,'' As SUP_PRD_NO,B.UNIT, ");
            _sb.Append(" A.CAS_NO,A.TASK_ID,A.AMT_CLS,A.AMTN_NET_CLS,A.TAX_CLS,A.QTY_CLS, 0 As AMTN_EP1,0 As AMTN_ER1,B.REM AS REM1,A.CUS_NO,A.TAX_ID,'' as VOH_ID,'' IMTAG  ,A.TAX_ID,'' as VOH_ID,'' IMTAG,A.REM   ");
            _sb.Append(" From MF_TD A,TF_TD B Left Outer Join PRDT G On B.PRD_NO=G.PRD_NO  ");

            GetSQLStrForDT(_sb, BIL_ID, TURN_ID, DEP, SAL_NO, CUS_NO, INCUS, CAS_NO, TASK_ID, CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE, Zhang_ID, CUR_ID, TAX_ID);
            if (CHK_DATE == "Y")
                _sb.Append(" and (A.TD_DD BETWEEN '" + Convert.ToDateTime(FDATE) + "' and '" + Convert.ToDateTime(EDATE) + "') ");
            if (!string.IsNullOrEmpty(FPS_NO))
                _sb.Append(" and (A.TD_NO>='" + FPS_NO + "')");
            if (!string.IsNullOrEmpty(FPS_NO))
                _sb.Append(" and (A.TD_NO<='" + EPS_NO + "') ");
            return _sb.ToString();

        }

        private void GetSQLStrForDT(StringBuilder _sb, string BIL_ID, string TURN_ID, string DEP, string SAL_NO, string CUS_NO, string INCUS, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE, string Zhang_ID, string CUR_ID, string TAX_ID)
        {
            _sb.Append(" Where (A.TD_NO=B.TD_NO) ");

            _sb.Append(" and (IsNull(A.LZ_CLS_ID,'')<>'T')");
            _sb.Append(" and ((A.TURN_ID='" + TURN_ID + "') Or (IsNull(A.TURN_ID,'')='')) ");
            if (!UpIsZero)
                _sb.Append(" And ( (IsNull(B.AMTN_NET,0)+IsNull(B.TAX,0))<>0 )  ");
            _sb.Append(" And (IsNull(A.Zhang_ID,'')='" + Zhang_ID + "' )  ");
            _sb.Append(" And (IsNull(A.KP_ID,'')<>'T') ");
        ...
        private string GetSqlStrForMF_TC(string BIL_ID, string CHK_DATE, string FDATE, string EDATE, string FPS_NO, string EPS_NO, string CUS_NO, string INCUS, string TAX_ID, string TURN_ID, string Zhang_ID, string DEP, string SAL_NO, string CUR_ID, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE)
        {
            StringBuilder _sb = new StringBuilder();
            _sb.Append(" SELECT B.PRD_NO,G.Name As PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
            _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TC_DD AS PAY_DD,'TC' As BIL_ID,A.TC_NO As BIL_NO, ");

            _sb.Append(" 0 As Dis_Cnt,'' As OS_NO,A.ZHANG_ID,0 As AMTN_EP,0 As AMTN_ER,B.UP,B.BAT_NO,G.CHK_TAX,'' As CUS_OS_NO,'' As SUP_PRD_NO,B.UNIT,");

            _sb.Append(" A.CAS_NO,A.TASK_ID,A.AMT_CLS,A.AMTN_NET_CLS,A.TAX_CLS,A.QTY_CLS, 0 As AMTN_EP1,0 As AMTN_ER1,B.REM AS REM1,A.CUS_NO,A.TAX_ID,'' as VOH_ID,'' IMTAG,A.REM   ");


            _sb.Append(" From MF_TC A,TF_TC B Left Outer Join PRDT G On B.PRD_NO=G.PRD_NO ");

            GetSQLStrForTC(_sb, BIL_ID, TURN_ID, DEP, SAL_NO, CUS_NO, INCUS, CAS_NO, TASK_ID, CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE, Zhang_ID, CUR_ID, TAX_ID);
            if (CHK_DATE == "Y")
                _sb.Append(" and (A.TC_DD BETWEEN '" + Convert.ToDateTime(FDATE) + "' and '" + Convert.ToDateTime(EDATE) + "') ");
            if (!string.IsNullOrEmpty(FPS_NO))
                _sb.Append(" and (A.TC_NO>='" + FPS_NO + "')");
            if (!string.IsNullOrEmpty(EPS_NO))
                _sb.Append(" and (A.TC_NO<='" + EPS_NO + "') ");
            return _sb.ToString();
        }

        private void GetSQLStrForTC(StringBuilder _sb, string BIL_ID, string TURN_ID, string DEP, string SAL_NO, string CUS_NO, string INCUS, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE, string Zhang_ID, string CUR_ID, string TAX_ID)
        {
            _sb.Append(" Where (A.TC_NO=B.TC_NO) ");
        ...
        private string GetSqlStrForMF_TB(string BIL_ID, string CHK_DATE, string FDATE, string EDATE, string FPS_NO, string EPS_NO, string CUS_NO, string INCUS, string TAX_ID, string TURN_ID, string Zhang_ID, string DEP, string SAL_NO, string CUR_ID, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE)
        {
            StringBuilder _sb = new StringBuilder();
            _sb.Append(" SELECT B.PRD_NO,B.PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
            _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.EST_ITM As PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TB_DD AS PAY_DD,'TB' As BIL_ID,A.TB_NO As BIL_NO,");
            _sb.Append(" 0 As Dis_Cnt,'' As OS_NO,A.ZHANG_ID,0 As AMTN_EP,0 As AMTN_ER,B.UP,B.BAT_NO,G.CHK_TAX,'' As CUS_OS_NO,'' As SUP_PRD_NO,B.UNIT, ");
            _sb.Append(" A.CAS_NO,A.TASK_ID,A.AMT_CLS,A.AMTN_NET_CLS,A.TAX_CLS,A.QTY_CLS, 0 As AMTN_EP1,0 As AMTN_ER1,B.REM AS REM1,A.CUS_NO, ");
            ...
            _sb.Append(" From MF_TB A,TF_TB B Left Outer Join PRDT G On B.PRD_NO=G.PRD_NO ");

            GetSQLStrForTB(_sb, BIL_ID, TURN_ID, DEP, SAL_NO, CUS_NO, INCUS, CAS_NO, TASK_ID, CHKPRD, FPRD_NO, EPRD_NO, CHKBilType, FBILTYPE, EBILTYPE, Zhang_ID, CUR_ID, TAX_ID);
            if (CHK_DATE == "Y")
                _sb.Append(" and (A.TB_DD BETWEEN '" + Convert.ToDateTime(FDATE) + "' and '" + Convert.ToDateTime(EDATE) + "') ");
            if (!string.IsNullOrEmpty(FPS_NO))
                _sb.Append(" and (A.TB_NO>='" + FPS_NO + "')");
            if (!string.IsNullOrEmpty(EPS_NO))
                _sb.Append(" and (A.TB_NO<='" + EPS_NO + "') ");
            return _sb.ToString();

        }

        private void GetSQLStrForTB(StringBuilder _sb, string BIL_ID, string TURN_ID, string DEP, string SAL_NO, string CUS_NO, string INCUS, string CAS_NO, string TASK_ID, string CHKPRD, string FPRD_NO, string EPRD_NO, string CHKBilType, string FBILTYPE, string EBILTYPE, string Zhang_ID, string CUR_ID, string TAX_ID)
        {
            _sb.Append(" Where (A.TB_NO=B.TB_NO) ");
        ...
        {
            string _sql = "UPDATE MF_LZ1 SET CHK_MAN = @CHK_MAN, CLS_DATE = @CLS_DATE WHERE LZ_ID = @LZ_ID AND LZ_NO = @LZ_NO";
        ...
        {
            string _sqlStr = " UpDate MF_PSS Set ACC_FP_NO='' Where ACC_FP_NO=@LZ_NO And "+
                              " Exists(Select B.CK_NO From TF_CK B Where B.CK_ID='CK' And B.CK_NO=@CK_NO And B.OS_ID='SA' And B.OS_NO=MF_PSS.PS_NO))";
            SqlParameter[] _aryPt = new SqlParameter[2];
            _aryPt[0] = new SqlParameter("@LZ_NO", SqlDbType.VarChar, 20);
            _aryPt[0].Value = slzNo;
        ...
        {
            string _sql = "update MF_LZ1 set ARP_NO=@ArpNo,AMTN_NET=@Amtn_net,TAX=@Tax,AMT=@Amt where LZ_ID=@LzId and LZ_NO=@LzNo";
        ...
        public void UpDateMFTBHead(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TB set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID Where TB_NO=@TB_NO ";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[4];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@INV_NO", SqlDbType.VarChar, 14);
            _spc[0].Value = sourceDr["INV_NO"].ToString();
            _spc[1] = new System.Data.SqlClient.SqlParameter("@TURN_ID", SqlDbType.VarChar, 1);
            _spc[1].Value = sourceDr["TURN_ID"].ToString(); 

            _spc[2] = new System.Data.SqlClient.SqlParameter("@LZ_CLS_ID", SqlDbType.VarChar, 1);
            _spc[2].Value = sourceDr["LZ_CLS_ID"].ToString();

            _spc[3] = new System.Data.SqlClient.SqlParameter("@TB_NO", SqlDbType.VarChar, 20);
            _spc[3].Value = bilno; 
            this.ExecuteNonQuery(_sql, _spc);
        }
        /// <summary>
        ...
        /// <param name="bilno">来源单号</param>
        /// <param name="tblName"></param>
        public void UpDateMFTCHead(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TC set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID Where TC_NO=@TC_NO ";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[4];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@INV_NO", SqlDbType.VarChar, 14);
            _spc[0].Value = sourceDr["INV_NO"].ToString();
            _spc[1] = new System.Data.SqlClient.SqlParameter("@TURN_ID", SqlDbType.VarChar, 1);
            ...
            _spc[3] = new System.Data.SqlClient.SqlParameter("@TC_NO", SqlDbType.VarChar, 20);
            _spc[3].Value = bilno;
            this.ExecuteNonQuery(_sql, _spc);
        }
        ...
        /// <param name="bilno">来源单号</param>
        /// <param name="tblName"></param>
        public void UpDateMFDTHead(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TD set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID Where TD_NO=@TD_NO ";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[4];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@INV_NO", SqlDbType.VarChar, 14);
            _spc[0].Value = sourceDr["INV_NO"].ToString();
            _spc[1] = new System.Data.SqlClient.SqlParameter("@TURN_ID", SqlDbType.VarChar, 1);
            ...
            _spc[3] = new System.Data.SqlClient.SqlParameter("@TD_NO", SqlDbType.VarChar, 20);
            _spc[3].Value = bilno;
            this.ExecuteNonQuery(_sql, _spc);
        }
        ...
        public void UpDateTFTBBody(DataRow sourceDr,string bilno)
        {
            string _sql = "update TF_TB set AMT_FP=@AMT_FP,AMTN_NET_FP=@AMTN_NET_FP,TAX_FP=@TAX_FP,QTY_FP=@QTY_FP Where TB_NO=@TB_NO and  EST_ITM=@EST_ITM";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[6];
            ...
            _spc[4] = new System.Data.SqlClient.SqlParameter("@TB_NO", SqlDbType.VarChar, 20);
            _spc[4].Value = bilno; 

            _spc[5] = new System.Data.SqlClient.SqlParameter("@EST_ITM", SqlDbType.VarChar, 1);
            _spc[5].Value = sourceDr["EST_ITM"].ToString();
        ...
        /// <param name="sourceDr"></param>
        /// <param name="bilno"></param>
        public void UpDateTFTCBody(DataRow sourceDr, string bilno)
        {
            string _sql = "update TF_TC set AMT_FP=@AMT_FP,AMTN_NET_FP=@AMTN_NET_FP,TAX_FP=@TAX_FP,QTY_FP=@QTY_FP Where TC_NO=@TC_NO and  PRE_ITM=@EST_ITM";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[6];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@AMT_FP", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["AMT_FP"].ToString()))
                _spc[0].Value = Convert.ToDecimal(sourceDr["AMT_FP"]);
            ...
            _spc[4] = new System.Data.SqlClient.SqlParameter("@TC_NO", SqlDbType.VarChar, 20);
            _spc[4].Value = bilno;
        ...
        /// <param name="sourceDr"></param>
        /// <param name="bilno"></param>
        public void UpDateTFDTBody(DataRow sourceDr, string bilno)
        {
            string _sql = "update TF_TD set AMT_FP=@AMT_FP,AMTN_NET_FP=@AMTN_NET_FP,TAX_FP=@TAX_FP,QTY_FP=@QTY_FP Where TD_NO=@TD_NO and  PRE_ITM=@EST_ITM";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[6];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@AMT_FP", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["AMT_FP"].ToString()))
                _spc[0].Value = Convert.ToDecimal(sourceDr["AMT_FP"]);
            ...
            _spc[4] = new System.Data.SqlClient.SqlParameter("@TD_NO", SqlDbType.VarChar, 20);
            _spc[4].Value = bilno;
        ...
        /// <summary>回写托工缴回单表头栏位滙总合并
        /// 回写托工缴回单表头栏位滙总合并
        /// </summary>
        /// <param name="sourceDr">来源单DataRow</param>
        /// <param name="bilno">来源单号</param>
        /// <param name="tblName"></param>
        public void UpDateMFTBHead2(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TB set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID,AMT_CLS=@AMT_CLS,AMTN_NET_CLS=@AMTN_NET_CLS,TAX_CLS=@TAX_CLS,QTY_CLS=@QTY_CLS Where TB_NO=@TB_NO ";
            ...
            _spc[3] = new System.Data.SqlClient.SqlParameter("@AMT_CLS", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["AMT_CLS"].ToString()))
                _spc[3].Value = Convert.ToDecimal(sourceDr["AMT_CLS"].ToString());
            else
                _spc[3].Value = DBNull.Value;


            _spc[4] = new System.Data.SqlClient.SqlParameter("@AMTN_NET_CLS", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["AMTN_NET_CLS"].ToString()))
                _spc[4].Value = Convert.ToDecimal(sourceDr["AMTN_NET_CLS"].ToString());
            else
                _spc[4].Value = DBNull.Value;

            _spc[5] = new System.Data.SqlClient.SqlParameter("@TAX_CLS", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["TAX_CLS"].ToString()))
                _spc[5].Value = Convert.ToDecimal(sourceDr["TAX_CLS"].ToString());
            else
                _spc[5].Value = DBNull.Value;

            _spc[6] = new System.Data.SqlClient.SqlParameter("@QTY_CLS", SqlDbType.Decimal);
            if (!string.IsNullOrEmpty(sourceDr["QTY_CLS"].ToString()))
                _spc[6].Value = Convert.ToDecimal(sourceDr["QTY_CLS"].ToString());
            else
                _spc[6].Value = DBNull.Value;

            _spc[7] = new System.Data.SqlClient.SqlParameter("@TB_NO", SqlDbType.VarChar, 20);
            _spc[7].Value = bilno;
            this.ExecuteNonQuery(_sql, _spc);
        }

        /// <summary>回写托工退回单表头栏位滙总合并
        /// 回写托工退回单表头栏位滙总合并
        /// </summary>
        /// <param name="sourceDr">来源单DataRow</param>
        /// <param name="bilno">来源单号</param>
        /// <param name="tblName"></param>
        public void UpDateMFTCHead2(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TC set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID,AMT_CLS=@AMT_CLS,AMTN_NET_CLS=@AMTN_NET_CLS,TAX_CLS=@TAX_CLS,QTY_CLS=@QTY_CLS Where TC_NO=@TC_NO ";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[8];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@INV_NO", SqlDbType.VarChar, 14);
            _spc[0].Value = sourceDr["INV_NO"].ToString();
            _spc[1] = new System.Data.SqlClient.SqlParameter("@TURN_ID", SqlDbType.VarChar, 1);
            ...
            _spc[7] = new System.Data.SqlClient.SqlParameter("@TC_NO", SqlDbType.VarChar, 20);
            _spc[7].Value = bilno;
            this.ExecuteNonQuery(_sql, _spc);
        }


        /// <summary>回写托工折让单表头栏位滙总合并
        /// 回写托工折让单表头栏位滙总合并
        /// </summary>
        /// <param name="sourceDr">来源单DataRow</param>
        /// <param name="bilno">来源单号</param>
        /// <param name="tblName"></param>
        public void UpDateMFDTHead2(DataRow sourceDr, string bilno)
        {
            string _sql = "update MF_TD set INV_NO=@INV_NO,TURN_ID=@TURN_ID,LZ_CLS_ID=@LZ_CLS_ID,AMT_CLS=@AMT_CLS,AMTN_NET_CLS=@AMTN_NET_CLS,TAX_CLS=@TAX_CLS,QTY_CLS=@QTY_CLS Where TD_NO=@TD_NO ";
            System.Data.SqlClient.SqlParameter[] _spc = new System.Data.SqlClient.SqlParameter[8];
            _spc[0] = new System.Data.SqlClient.SqlParameter("@INV_NO", SqlDbType.VarChar, 14);
            _spc[0].Value = sourceDr["INV_NO"].ToString();
            _spc[1] = new System.Data.SqlClient.SqlParameter("@TURN_ID", SqlDbType.VarChar, 1);
            ...
            _spc[7] = new System.Data.SqlClient.SqlParameter("@TD_NO", SqlDbType.VarChar, 20);
            _spc[7].Value = bilno;
            this.ExecuteNonQuery(_sql, _spc);
        }
        ...
        {
            string _sql = "Select Top 1 LZ_NO From TF_LZ1 Where (LZ_ID=@LZ_ID) And (BIL_ID=@BIL_ID) And (CK_NO=@CK_NO)";
        ...
        {
            string _sql = "Select Top 1 LZ_NO From TF_LZ1 Where (LZ_ID in ('LI','LP')) And (BIL_ID=@BIL_ID) And (PS_NO=@PS_NO)";
            SqlParameter[] _spc = new SqlParameter[2];

            _spc[0] = new SqlParameter("@BIL_ID", SqlDbType.Char, 2);
            _spc[0].Value = bilId;

            _spc[1] = new SqlParameter("@PS_NO", SqlDbType.VarChar, 20);
            _spc[1].Value = ckno;

            SunlikeDataSet _ds = new SunlikeDataSet();
            this.FillDataset(_sql, _ds, new string[] { "TF_LZ1" }, _spc);
            return _ds;
        }

        #region 更新销货立帐开票单凭证号码
        /// <summary>
        /// 更新销货立帐开票单凭证号码
        /// </summary>
        ...
        public bool UpdateVohNo(string lzId, string lzNo, string vohNo)
        {
            bool _result = false;
            string _sqlStr = "";
            _sqlStr = " UPDATE MF_LZ1 SET VOH_NO=@VOH_NO WHERE LZ_ID=@LZ_ID AND LZ_NO=@LZ_NO";
        ...
        {
            string _sql = "";
            if (bilId == "PC" || bilId == "PB" || bilId == "BN" || bilId == "BB")
            {
                _sql = " SELECT Distinct B.BIL_NO,0 As EST_ITM,B.BIL_ID,H.REM1 As Inv_NO,  " +
                         "  IsNull(H.Amtn_Net_FP,0)+IsNull(H.Tax_FP,0) As FP_AMTN From TF_BIL B,MF_BIL H " +
                         " Where (H.BIL_ID='BW' Or H.BIL_ID='BP') And (B.REM3='"+bilId+"') And (B.REM1=@BILNO) " +
                         " And (IsNull(H.REM1,'')<>'') And (H.BIL_ID=B.BIL_ID) And (H.BIL_NO=B.BIL_NO) " +
                         " UNION All " +
                         " SELECT Distinct B.LZ_NO As BIL_NO,B.EST_ITM,B.LZ_ID As BIL_ID,A.INV_NO, " +
                         " (IsNull(B.AMTN_NET,0)+IsNull(B.Tax,0)) As FP_AMTN  " +
                         " From TF_LZ1 B, MF_LZ1 A " +
                         " Where (B.BIL_ID='"+bilId+"') And (B.PS_NO=@BILNO)  " +
                         " And (B.LZ_ID=A.LZ_ID) And (B.LZ_NO=A.LZ_NO) And (IsNull(A.Inv_No,'')<>'') And (IsNull(A.CHK_MAN,'')<>'') ";
            }
        ...
        //private string GetSqlStrForMF_DT(Dictionary<string, object> ht)
        //{
        //    StringBuilder _sb = new StringBuilder();
        //    _sb.Append(" SELECT B.PRD_NO,G.Name As PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
        //    _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TD_DD AS PAY_DD,'DT' As BIL_ID,A.TD_NO As BIL_NO, ");
        //    _sb.Append(" 0 As Dis_Cnt,'' As OS_NO,A.ZHANG_ID,0 As AMTN_EP,0 As AMTN_ER,UP=0,B.BAT_NO,G.CHK_TAX,'' As CUS_OS_NO,'' As SUP_PRD_NO,B.UNIT, ");
        //    _sb.Append(" A.CAS_NO,A.TASK_ID,A.AMT_CLS,A.AMTN_NET_CLS,A.TAX_CLS,A.QTY_CLS, 0 As AMTN_EP1,0 As AMTN_ER1,B.REM AS REM1,A.CUS_NO,A.TAX_ID,'' as VOH_ID,'' IMTAG  ,A.TAX_ID,'' as VOH_ID,'' IMTAG   ");
        //    _sb.Append(" From MF_TD A,TF_TD B Left Outer Join PRDT G On B.PRD_NO=G.PRD_NO  ");

        //    GetSQLStrForDT(_sb, ht);
        //    if (ht["CHK_DATE"].ToString() == "Y")
        //        _sb.Append(" and (A.TD_DD BETWEEN '" + Convert.ToDateTime(ht["FDATE"].ToString()) + "' and '" + Convert.ToDateTime(ht["EDATE"].ToString()) + "') ");
        //    if (!string.IsNullOrEmpty(ht["FPS_NO"].ToString()))
        //        _sb.Append(" and (A.TD_NO>='" + ht["FPS_NO"].ToString() + "')");
        //    if (!string.IsNullOrEmpty(ht["EPS_NO"].ToString()))
        //        _sb.Append(" and (A.TD_NO<='" + ht["EPS_NO"].ToString() + "') ");
        //    return _sb.ToString();
        //}


        //private void GetSQLStrForDT(StringBuilder _sb, Dictionary<string, object> ht)
        //{
        //    _sb.Append(" Where (A.TD_NO=B.TD_NO) ");
        ...
        //    if (!UpIsZero)
        //        _sb.Append(" And (IsNull(B.AMTN_NET,0)+IsNull(B.TAX,0)<>0 )  ");
        //    _sb.Append(" And (IsNull(A.Zhang_ID,'')='" + ht["Zhang_ID"].ToString() + "' )  ");
        //    _sb.Append(" And (IsNull(A.KP_ID,'')<>'T') ");
        ...
        //private string GetSqlStrForMF_TC(Dictionary<string, object> ht)
        //{
        //    StringBuilder _sb = new StringBuilder();
        //    _sb.Append(" SELECT B.PRD_NO,G.Name As PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
        //    _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TC_DD AS PAY_DD,'TC' As BIL_ID,A.TC_NO As BIL_NO, ");
        ...
        //    _sb.Append(" A.CAS_NO,A.TASK_ID,A.AMT_CLS,A.AMTN_NET_CLS,A.TAX_CLS,A.QTY_CLS, 0 As AMTN_EP1,0 As AMTN_ER1,B.REM AS REM1,A.CUS_NO,A.TAX_ID,'' as VOH_ID,'' IMTAG   ");


        //    _sb.Append(" From MF_TC A,TF_TC B Left Outer Join PRDT G On B.PRD_NO=G.PRD_NO ");

        //    GetSQLStrForTC(_sb, ht);
        //    if (ht["CHK_DATE"].ToString() == "Y")
        //        _sb.Append(" and (A.TC_DD BETWEEN '" + Convert.ToDateTime(ht["FDATE"].ToString()) + "' and '" + Convert.ToDateTime(ht["EDATE"].ToString()) + "') ");
        //    if (!string.IsNullOrEmpty(ht["FPS_NO"].ToString()))
        //        _sb.Append(" and (A.TC_NO>='" + ht["FPS_NO"].ToString() + "')");
        //    if (!string.IsNullOrEmpty(ht["EPS_NO"].ToString()))
        //        _sb.Append(" and (A.TC_NO<='" + ht["EPS_NO"].ToString() + "') ");
        //    return _sb.ToString();
        //}


        //private void GetSQLStrForTC(StringBuilder _sb, Dictionary<string, object> ht)
        //{
        //    _sb.Append(" Where (A.TC_NO=B.TC_NO) ");
        ...
        //private string GetSqlStrForMF_TB(Dictionary<string, object> ht)
        //{
        //    StringBuilder _sb = new StringBuilder();
        //    _sb.Append(" SELECT B.PRD_NO,B.PRD_NAME,B.PRD_MARK,B.AMTN_NET,B.TAX,B.AMT,B.AMT_FP,B.AMTN_NET_FP, ");
        //    _sb.Append(" B.TAX_FP,B.QTY_FP,B.ITM,B.QTY,B.EST_ITM As PRE_ITM,G.SPC_TAX As TAX_RTO,A.EXC_RTO,A.TB_DD AS PAY_DD,'TB' As BIL_ID,A.TB_NO As BIL_NO,");
        ...
        //    GetSQLStrForTB(_sb, ht);
        //    if (ht["CHK_DATE"].ToString() == "Y")
        //        _sb.Append(" and (A.TB_DD BETWEEN '" + Convert.ToDateTime(ht["FDATE"].ToString()) + "' and '" + Convert.ToDateTime(ht["EDATE"].ToString()) + "') ");
        //    if (!string.IsNullOrEmpty(ht["FPS_NO"].ToString()))
        //        _sb.Append(" and (A.TB_NO>='" + ht["FPS_NO"].ToString() + "')");
        //    if (!string.IsNullOrEmpty(ht["EPS_NO"].ToString()))
        //        _sb.Append(" and (A.TB_NO<='" + ht["EPS_NO"].ToString() + "') ");
        //    return _sb.ToString();
        //}


        //private void GetSQLStrForTB(StringBuilder _sb, Dictionary<string, object> ht)
        //{
        //    _sb.Append(" Where (A.TB_NO=B.TB_NO) ");
